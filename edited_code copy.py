...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, dataset):
    logger.info(
        f"Running validation... \n Generating images for {len(dataset)} pre-disaster scenarios."
    )
    ...
    for i in range(len(dataset)):
        pre_image_data = dataset[i]
        pre_image_text = pre_image_data['input_ids']
        pre_image = pre_image_data['pre_pixel_values'].unsqueeze(0).to(accelerator.device)

        with torch.autocast("cuda"):
            post_image_generated = pipeline(pre_image_text, num_inference_steps=25, generator=generator, guidance_scale=7.5).images[0]

        # Convert tensors to PIL Images and save
        pre_image_pil = to_pil_image(pre_image[0].cpu())  # Assuming pre_image is a tensor with shape [C, H, W]
        post_image_pil = to_pil_image(post_image_generated.cpu())  # post_image_generated should be a tensor with shape [C, H, W]
        
        save_directory = "./finaldata/testimgs/"
        pre_image_pil.save(os.path.join(save_directory, f"pre_disaster_{i}.png"))
        print("saved pre_disaster_")
        post_image_pil.save(os.path.join(save_directory, f"post_disaster_generated_{i}.png"))
        print("saved post_disaster_generated_")

        validation_images.append((pre_image.cpu(), post_image_generated))        
...
def main():
    ...
    import wandb
    wandb.init(project="img2img", dir=args.output_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            images = log_validation( text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch, dataset), 
